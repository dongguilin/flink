 ...
 *
 * source(kafka, json格式)
 * ..........===>  (inner join LATERAL TABLE (user_udtf(o.log_id, o.user_no)), group by hop window)
 * .................===> sink(kafka, json格式)
 *
 * 3) 最终执行计划:
 *
...
object Demo_InnerJoin_1 {
  ...
  def main(args: Array[String]): Unit = {
     ...
     *
     * 输出:
     * {"hop_start":1651112400000,"hop_end":1651113000000,"hop_proctime":"2022-04-28 10:30:00.009","user_no":"0001","user_name":"小伊","gender":"女","age":20,"city":"郑州","error_times":6}
     */
    tEnv.connect(
      new Kafka()
        .version("0.11")
    ...
    val kafkaTable = tEnv.sqlQuery(
      """
        ...
        |FROM
        |   kafka_source_table o, LATERAL TABLE (user_udtf(o.log_id, o.user_no)) AS T(user_name, gender, age, city)
        |WHERE
    ...
    kafkaTable.insertInto("kafka_sink_table")
