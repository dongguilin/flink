...
public class LastColUdaf extends AggregateFunction<String, LastColUdaf.Accum> {
    ...
    public void accumulate(Accum accumulator, LocalDateTime col, LocalDateTime timestamp) {
        if (accumulator.timestamp == null || timestamp.isAfter(accumulator.timestamp)) {
            accumulator.value = col.format(formatter);
    ...
    public void merge(Accum accumulator, Iterable<Accum> it) {
        ...
        while (iterator.hasNext()) {
            ...
            if (b.orderKey == null) {
                if (accumulator.timestamp == null || b.timestamp.isAfter(accumulator.timestamp)) {
